...
def create_promo(promo_data):
    # Create a new promo
    promo_ref = promos_ref.document(promo_data['id'])
    promo_ref.set(promo_data)
    return promo_data

def get_promo(promo_id):
    # Get a promo
    promo_ref = promos_ref.document(promo_id)
    promo = promo_ref.get()
    if promo.exists:
...
def get_promos(sme_id):
    ...
    promos = promos_ref.where('sme_id', '==', sme_id).stream()
...
def delete_promo(promo_id):
    ...
    promo_ref = promos_ref.document(promo_id)
    promo_ref.delete()
    return "promo_id: " + promo_id + " deleted"